...
<WarningBox>
You will also see a **service role key** on this page. That key has full admin access to your database. Never use it in your frontend code. For now, you only need the anon key, which has limited permissions enforced by Row Level Security.
</WarningBox>
...
> Install @supabase/supabase-js and create a Supabase client
  ...
  environment variables for the URL and anon key:
...
In Next.js, environment variables prefixed with `NEXT_PUBLIC_` are available in the browser. Variables without this prefix are only available on the server. Since the Supabase client runs in the browser, we need the prefix. For server-only secrets (like the service role key), do not use the prefix.
</Callout>
...
<WarningBox>
The MCP server uses the service role key (not the anon key) because it needs full database access. This key should only be in your local settings file, never committed to Git. Make sure `.claude/` is in your `.gitignore` or that `settings.json` does not get committed.
</WarningBox>
